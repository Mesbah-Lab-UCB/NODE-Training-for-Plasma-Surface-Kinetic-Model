...
def ConcatSimData(df, tsim, x_data, D, v):
    ...
    new_data = torch.tensor(df[df.columns[1:]].values, dtype=torch.float32)
...
def MapTime(fmin, f_current, tsim, tstart=0):
    ...
    """
    tmin = np.linspace(tstart,tsim,int(fmin))
    t = np.linspace(tstart,tsim,f_current)
...
def DataPreSiALE(path_to_dir, list_of_impacts, list_of_energies, y0_internal, skiprows=False, last_row = 0, tsim=30.0, Diffusion=1e-21, v_guess=1e-12):
    ...
    """
        ...
        The .csv datafile must have the columns as ['Time','Surface','Mixed Layer','Total','Surface Prob','Mixed Layer Prob']
    ...
    x = torch.empty((0, 0, int(min(list_of_impacts_num)), prob_no))
    
    # Dim 0: energies   Dim 1: fluence   Dim 1: probability and internal points
    for f in list_of_impacts:
        x_data = torch.empty((0, int(f), prob_no+1)) # +1 for the total no of F atoms
        ...
        for e in list_of_energies:
            ...
            # Drop the surface and bulk columns
            df.drop(['Surface', 'Mixed Layer'], axis=1, inplace=True)
            
            #df.rename(columns={"Bulk Prob":"Mixed Layer Prob"}, inplace=True)
            df['Bulk Prob'] = 1 - (df['Surface Prob']+df['Mixed Layer Prob'])
            
            # Reintroducing the initial configuration of pure Si
            # df.loc[-1] = np.zeros(len(df.columns))
            # df.index = df.index + 1
            # df = df.sort_index()
            ...
            # Reposition column for total number
            # The last column will be the total column, following the arrangement in SiALE_ODEFunc
            Nf = (df.pop('Total') + 1e-6) / 3.
            
            # If you need to normalize the total number of F atoms
            # Nmax = df['Total'].max()
            # Nf = (df.pop('Total') + 1e-6) / Nmax
            
            df.insert(len(df.columns), 'Total', Nf)
    ...
    '---For RIE, if initial value is taken as t=0, then we have singularity issues---'
...
def Total_F(t, args):
    D, v, J_f, A = args
    return (D / v**2) * (J_f * A) * t + 1, (D / v**2) * J_f * A
...
def SiALE_ODEfunc(t, y, args):
    ...
    """
        ...
        y: 2D pytorch tensor of variable values. Dim 0 corresponds to the different
            values of the ion energy, Dim 1 to the fluence values, and Dim 2 corresponds to the different variables. 
            For n internal grid points, shape of Dim 2 is n+3
    ...
    tsim, D, v_guess, A, n_Si, n_Surface, flux_ratio, delX = params
    
    # Unpack NN models for the rate constants
    kf1, kf2, kf3, ks3 = rate_constants
    ...
    flux = fluence / tsim
    J_Ar = flux / (flux_ratio+1)
    J_F = flux * flux_ratio / (flux_ratio + 1)
    ...
    kf1_out = kf1(model_input)
    kf1_val = kf1_out.reshape(l,m)
    kf2_out = kf2(model_input)
    kf2_val = kf2_out.reshape(l,m)
    kf3_out = kf3(model_input)
    kf3_val = kf3_out.reshape(l,m)
    ks3_out = ks3(model_input)
    ks3_val = ks3_out.reshape(l,m)
    
    # Total F atoms at time t and rate of F atom entry
    Nf, dNfdt = Total_F(t, [D, v_guess, J_F, A])
    
    # F atomic fraction on the surface
    c2 = (Nf.clone() * y[...,N].clone()) / (n_Surface * A)
    
    # Velocity of etch front
    # v = (J_Ar / n_Si) * ((kf1_val + kf3_val - ks3_val) * Nf.clone() * y[...,N].clone() + ks3_val * n_Surface * A)
    v = (J_Ar * n_Surface * A / n_Si) * ((kf1_val + kf3_val - ks3_val) * c2 + ks3_val)
    ...
    alpha = J_Ar * kf1_val * torch.mul(Nf.clone() , y[...,N]) / (v * n_Si) - y[...,0]
    # alpha = J_Ar * kf1_val * torch.mul(Nf.clone() , y[...,N]) * impact_vec / (v * n_Si) - y[...,0]
    ...
    # Surface ODE
    func[...,N] =  -(y[...,N].clone() / Nf.clone()) * dNfdt.clone() - J_Ar * A * y[...,N].clone() * (kf1_val + kf3_val) * D / v**2 + \
                    y[...,N+1]* kf2_val * n_Surface * A * (1 - c2) * D / (v**2) + \
                        n_Si * A * y[...,0].clone() * D / (Nf.clone() * v)   
    # func[...,N] =  -(y[...,N].clone() / Nf.clone()) * dNfdt.clone() - J_Ar * A * y[...,N].clone() * (kf1_val + kf3_val) * D / v**2 + \
    #                 y[...,N+1]* kf2_val * n_Surface * A * (1 - c2) * D / (v**2) + \
    #                     n_Si * A * y[...,0].clone() * D / (impact_vec * Nf.clone() * v)            

             
     # Bulk Gas ODE, decoupled from the rest of the equations. Used for plotting and sanity checks.
    func[...,N+1] = (1- y[...,N+1].clone()) * dNfdt.clone() / Nf.clone() + J_Ar * A * kf3_val * y[...,N].clone() * D / v**2 - \
                    y[...,N+1].clone() * kf2_val * n_Surface * A * (1 - c2) * D / (v**2)
    
                    
    # Mixed Layer ODE. This is also decoupled from the rest of the equations. Used for plotting and sanity checks.
    func[...,N+2] = -(y[...,N+2].clone() / Nf.clone()) * dNfdt.clone() - \
        n_Si * A * D * ((y[...,1].clone() - y_at_minus1.clone()) / (2 * delX)) / (Nf.clone() * v)
    # func[...,N+2] = -(y[...,N+2].clone() / Nf.clone()) * dNfdt.clone() - \
    #     n_Si * A * D * ((y[...,1].clone() - y_at_minus1.clone()) / (2 * delX)) / (impact_vec * Nf.clone() * v)
